...
static f8080State   i8080_state_ldax_M2T2;
static f8080State   i8080_state_ldax_M2TW;
static f8080State   i8080_state_ldax_M2T3;
...
static f8080State   i8080_state_stax_M2T2;
static f8080State   i8080_state_stax_M2T3;
...
// i8080_ldst_init: link up the state functions for LOAD and STORE ops
...
// i8080_state_ldax_M1T4 for each register pair:
// finish opcode decode, directing execution to the proper M2 T1 handler.

#define	IMPL_LDAX_M1T4(RH,RL)                                           \
    static DECL_LDAX_M1T4(RH,RL) {                                      \
        switch (phase) {                                                \
        case PHI1_RISE:                                                 \
            break;                                                      \
        case PHI2_RISE:                                                 \
...
// i8080_state_ldax_M2T1 for each register pair:
// initiate memory read with RP providing the address.
// logic continues in the M2T2 handler.
...
#define	IMPL_LDAX_M2T1(RH,RL)	static DECL_LDAX_M2T1(RH,RL) {          \
        ...
        case PHI2_RISE:                                                 \
            ...
            break;                                                      \
        case PHI2_FALL:                                                 \
            cpu->state_next = i8080_state_ldax_M2T2;                    \
            break;                                                      \
        }                                                               \
    }                                                                   \
    DEFN_LDAX_M2T1(RH,RL)
...
// i8080_state_ldax_M2T2: continue memory read for LDAX

static void i8080_state_ldax_M2T2(i8080 cpu, int phase)
{
    switch (phase) {
      case PHI1_RISE:
          break;
      ...
      case PHI2_FALL:
          if (VAL(READY))
              cpu->state_next = i8080_state_ldax_M2T3;
          else
              cpu->state_next = i8080_state_ldax_M2TW;
          break;
    }
}

// i8080_state_ldax_M2TW: memory wait for LDAX

static void i8080_state_ldax_M2TW(i8080 cpu, int phase)
{
    switch (phase) {
      case PHI1_RISE:
          RAISE(WAIT);
...
// i8080_state_ldax_M2T3: finish memory read for LDAX

static void i8080_state_ldax_M2T3(i8080 cpu, int phase)
{
    switch (phase) {
      case PHI1_RISE:
          LOWER(WAIT);
...
#define	IMPL_STAX_M1T4(RH,RL)                                           \
    static DECL_STAX_M1T4(RH,RL) {                                      \
        switch (phase) {                                                \
        case PHI1_RISE:                                                 \
            break;                                                      \
        case PHI2_RISE:                                                 \
...
#define	IMPL_STAX_M2T1(RH,RL)                                           \
    static DECL_STAX_M2T1(RH,RL) {                                      \
        switch (phase) {                                                \
        case PHI1_RISE:                                                 \
            break;                                                      \
        case PHI2_RISE:                                                 \
            ...
            break;                                                      \
        case PHI2_FALL:                                                 \
            cpu->state_next = i8080_state_stax_M2T2;                    \
            break;                                                      \
        }                                                               \
    }                                                                   \
    DEFN_STAX_M2T1(RH,RL)
...
// i8080_state_stax_M2T2: 

static void i8080_state_stax_M2T2(i8080 cpu, int phase)
{
    switch (phase) {
      case PHI1_RISE:
          break;
      case PHI2_RISE:
          ...
          break;
      case PHI2_FALL:
          cpu->state_next = i8080_state_stax_M2T3;
          break;
    }
}

static void i8080_state_stax_M2T3(i8080 cpu, int phase)
{
    switch (phase) {
      case PHI1_RISE:
          RAISE(RETM1_INT);
